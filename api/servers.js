...
module.exports.load = async function (app, db) {
      ...
      queue.addJob(async (cb) => {
        ...
        if (
          ...
          if (!isNaN(ram) && !isNaN(disk) && !isNaN(cpu)) {
            ...
            newpterodactylinfo.relationships.servers.data.push(text);
  ...
  app.get("/reinstall", async (req, res) => {
    ...
    if (!req.query.id) return res.redirect("/dashboard?error=MISSINGID");

    // Verify server ownership
    const cacheaccount = await getPteroUser(req.session.userinfo.id, db);
    if (!cacheaccount) return res.redirect("/dashboard?error=INVALIDSERVER");

    // Check if the server exists in the user's server list
    if (!cacheaccount.attributes.relationships.servers.data) {
      return res.redirect("/dashboard?error=INVALIDSERVER");
    }

    let server = cacheaccount.attributes.relationships.servers.data.find(
      server => server.attributes.id === req.query.id
    );
    
    if (!server) return res.redirect("/dashboard?error=INVALIDSERVER");

    try {
      // Trigger reinstall via Pterodactyl API
      const response = await fetch(
        `${settings.pterodactyl.domain}/api/application/servers/${req.query.id}/reinstall`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) throw new Error(`Failed to reinstall server: ${response.statusText}`);

      return res.redirect(`/servers/edit?id=${req.query.id}&success=REINSTALLING`);
    } catch (error) {
      console.error("Reinstall error:", error);
      return res.redirect(`/servers/edit?id=${req.query.id}&error=FAILEDREINSTALL`);
    }
  });
};
